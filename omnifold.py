...
def reweight(X, Y, w, model, filepath, fitargs, val_data=None):
    ...
    h=model.fit(X, Y, sample_weight=w, **fitargs, **val_dict,callbacks=[earlystop])
    ...
    #w *= np.clip(preds/(1 - preds), fitargs.get('weight_clip_min', 0.), fitargs.get('weight_clip_max', np.inf))
   
    #w *=np.clip(np.squeeze(np.nan_to_num(w)),10**-10,10**100)
    #woahgirl=np.clip(np.squeeze(np.nan_to_num(w)),10.0**-10,10.0**50)
...
def omnifold(X_gen_i, Y_gen_i, X_det_i, Y_det_i, wdata, winit, det_model, mc_model, fitargs, 
    ...
    for i in range(it):
        ...
        rw = reweight(X_det_train, Y_det_train, w_train, model_det, model_det_fp_i,fitargs, val_data=(X_det_val, Y_det_val, w_val))[invperm_det]
        ...
        #np.clip(np.squeeze(np.nan_to_num(ws)),10**-10,10**100)
    ...
    return ws
    #return np.clip(np.squeeze(np.nan_to_num(ws)),10**-1,10**100)
